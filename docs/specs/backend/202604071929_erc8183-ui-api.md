...
## 6. GET `agent/commerce/stats`（统计图表）
...
| 接口 | 用途 | 数据来源 |
|------|------|---------|
| `GET /agent/commerce/scores` | Score 卡片 + 统计摘要 | `commerce_scores` / `commerce_scores_global`（触发器维护） |
| `GET /agent/commerce/actions` | 行为明细列表（表格） | `commerce_actions` |
| `GET /agent/identity/detail` | 含 `commerce_score` 便捷摘要 | 同 `commerce/scores` |
| **`GET /agent/commerce/stats`** | **统计图表（分布/趋势）** | **`commerce_actions`（纯计算，不依赖 scores 表）** |
...
| 接口 | 说明 |
|------|------|
| `GET /agent/commerce/stats` | 新增。提供 action 分布、时间序列趋势、Budget 分布，用于统计图表渲染。 |
...
| 接口 | 状态 |
|------|------|
| `GET /agent/commerce/scores` | 不变 |
| `GET /agent/commerce/actions` | 不变 |
...
- [ ] **Stats 接口新增验收**：
  - [ ] `GET /agent/commerce/stats?uid=X` 返回 HTTP 200，JSON 结构符合定义
